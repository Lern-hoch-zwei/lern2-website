...
import SchuelerVerwaltung from './SchuelerVerwaltung'
...
export default async function AdminSchuelerPage() {
  ...
  const { data: lehrkraefte } = await supabase
    .from('lehrkraefte')
  ...
  const { data: zuweisungen } = await supabase
    .from('lk_schueler')
    .select('id, lehrkraft_id, schueler_id')
    ...
    <main style={{ minHeight: '100vh', backgroundColor: '#F7F9FC', fontFamily: "'Inter', sans-serif", padding: '24px' }}>
      ...
      <div style={{ maxWidth: '960px', margin: '0 auto' }}>
        ...
        <SchuelerVerwaltung
          ...
          initialZuweisungen={zuweisungen || []}
        />
      </div>
    </main>
  )
